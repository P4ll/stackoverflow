# dataset paths
base_data = 'dataset/'
base_file_name = 'dataset/tmp/init_tmp.csv'
base_users_id_file = 'dataset/tmp/users_id.csv'
base_user_info_file = 'dataset/tmp/user_info.csv'
base_final_file = 'dataset/mdata.csv'
base_optimal_data = 'dataset/optimal_data.csv'

# Models data
base_model = 'D:/work/stackoverflow/models_data/'
base_model_lda = base_model + 'lda/'
base_model_ngram = base_model + 'ngrams/'